...
export default function Home() {
  ...
  <section className="flex flex-col items-center pt-14 pb-8 md:px-0 sm:px-0 px-4">
    <div>
      <h1 className="text-[30px] md:text-[30px] sm:text-[30px] text-[#1F2937] font-bold">Rise of Kingdoms Kingdoms Data</h1>
      <h2 className="text-center mt-2 mb-4 text-[20px] text-[#1F2937]">Data From 1093</h2>
    </div>

    <div id="menu" className="flex gap-8 items-center justify-center lg:justify-start flex-wrap">
      <input
        type="text"
        onChange={(e) => search(e.target.value)}
        className="bg-[#1F2937] shadow-lg outline-none p-4 rounded-lg text-white min-w-[300px] md:min-w-[500px] placeholder-white"
        placeholder="Search your Kingdom"
      />
      <select
        value={selectValue}
        onChange={(e) => optionChange(e.target.value)}
        className="min-w-[250px] shadow-lg rounded-lg bg-[#1F2937] text-white  outline-none p-4"
      >
      ...
      <button
        className="p-4 shadow-lg rounded-lg bg-[#1F2937] text-white"
        onClick={() => {
    ...
    <div id="info-box" className="flex flex-wrap justify-center gap-4 mt-10">
  ...
  <section>
                      ...
                      return (
                        <tr key={index} className="bg-opacity-20">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {" "}
                            {`${kingdom.Capped.toString().slice(0, 2)}.${kingdom.Capped.toString().slice(2, 5)} B`}{" "}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {" "}
                            {`${kingdom.Farmed.toString().slice(0, 2)}.${kingdom.Farmed.toString().slice(2, 5)} B`}{" "}
                          </td>
                        </tr>
                      );
                    })
    ...
    <div id="paginate-cover" className="flex flex-wrap justify-center md:justify-end ">
            {searchKey == "" ? 
            ( <ReactPaginate
              ...
              containerClassName="flex flex-wrap  justify-center gap-3 p-1"
